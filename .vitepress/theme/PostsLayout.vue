...
<template>
  <div class="md:border-l md:border-zinc-100 md:pl-6">
    <div class="flex max-w-3xl flex-col space-y-16">
      <article v-for="post in posts" :key="post.url" class="md:grid md:grid-cols-4 md:items-baseline">
        <div class="group relative flex flex-col items-start md:col-span-3">
          ...
          <time class="relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400 pl-3.5 md:hidden" :datetime="post.date">
            <span class="absolute inset-y-0 left-0 flex items-center" aria-hidden="true">
              <span class="h-4 w-0.5 rounded-full bg-zinc-200"></span>
            </span>
            {{ formatDate(post.date) }}
          </time>
          <p class="relative z-10 mt-2 text-sm text-zinc-600">{{ post.description }}</p>
        ...
        <time class="relative z-10 order-first mb-3 flex items-center text-sm text-zinc-400 mt-1 max-md:hidden" :datetime="post.date">
          {{ formatDate(post.date) }}
        </time>
      </article>
    </div>
